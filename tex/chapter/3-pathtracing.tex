...
To weigh the BRDF, the VNDF has first to be transformed into a probability distribution over the domain of incoming directions $\wi$ using the Jacobian of the reflection operator \bcite{walter2007,pharr2023}:
...
For the transmission component, I use the same approach, but with the Jacobian of the refraction operator \bcite{walter2007,pharr2023}:
...
\begin{equation}
    \begin{aligned}
        ...
        % &= \frac{G_1(\wo, \wm) D(\NdotH)}{|\LdotH + \VdotH / \eta|^2} \\
    \end{aligned}
\end{equation}
