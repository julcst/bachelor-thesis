...
Revisiting the rendering equation (\autoref{eq:rendering_equation}), we first concentrate on sampling the term $f(\wi, \vec{x}, \wo) \cos(\theta_i)$ of the integrand.
...
For my reference pathtracer, I will only employ BSDF sampling and direct light sampling (\autoref{sec:mis}).
...
To sample the VNDF, I use the method proposed by \textcite{dupuy2023}, which improves upon the original method \parencite{heitz2014a} and its follow-up \parencite{heitz2018}.
