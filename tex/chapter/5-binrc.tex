...
% TODO: Sample caustics more densely
...
The difference between the current count of light samples and the count at creation gives the number of total light samples $n$ that were used in the collection of flux for that query (\autoref{eq:pm}).
...
\begin{table}
    ...
    \begin{subtable}{.68\textwidth}
        ...
        \end{tabular}
    \end{subtable}
    \caption{The structs used to represent photons and queries.}
\end{table}
...
\textcite{jensen1996} originally proposed to use k-d-trees to store photons, to enable fast neighbor searches.
However, k-d-tree construction and querying are highly incoherent tasks and are commonly implemented using stacks.
GPU hardware neither includes a stack, nor does it support extensive branching, making k-d-trees for real-time rendering impractical.
