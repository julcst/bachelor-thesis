...
def create_loss_history_plot(prefix: str, tests: list[(str, str)], size = (7, 1)):
    ...
    for label, path in tests:
        with open(f"{prefix}{path}.hdr.json", "r") as f:
            ...
            ax.plot(iters, losses, label=label, linewidth=0.5)
    #ax.set_xlim(xmin=0)
    ax.set_xscale("log")
...
fig, ax = create_loss_history_plot("tests/quality_comparison/", [
    ("NRC+PT", "nrc+pt_1spp_thinker"),
    ("NRC+PT+SL", "nrc+pt+sl_1spp_thinker"),
    ("NRC+BT", "nrc+bt_1spp_thinker"),
    ("NRC+LT", "nrc+lt_1spp_thinker"),
    ("NRC+SPPC", "nrc+sppc_1spp_thinker")
])
